...
- Для полей `type: string` + `format: date-time` сгенерированный клиент сериализует значения в UTC ISO-8601 с микросекундами и суффиксом `Z`: `2020-01-01T15:47:21.000000Z`.
- Для полей `type: string` + `format: date` используется date-only формат `Y-m-d`.
