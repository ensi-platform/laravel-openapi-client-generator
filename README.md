...
Если при запуске команды вы получаете ошибку вида

```bash
188:         foreach (scandir($dir) as $fileWithoutDir) {
  scandir(/var/www/clients/client-php): Failed to open directory: No such file or directory
```

то вам необходимо пойти создать вручную соответсвующую директорию в файловой системе в соответствии с конфигурацией пакета. Обычно это `base_path('clients/client-php')` 
## Ограничения
